...
Текущая серверная реализация использует адаптер Cloudflare Worker/D1, но веб-приложение зависит только от HTTP-контракта. Его можно заменить другим сервером и БД, не переписывая интерфейс — подробности в [архитектурной документации](docs/ARCHITECTURE.md). Администраторские email задаются списком `ADMIN_EMAILS` в конфигурации Worker; порядок регистрации не влияет на выдачу роли.
...
- Пароли хешируются PBKDF2-SHA-256 (100 000 итераций — максимум Cloudflare Workers, индивидуальная соль).
