...
async function loadLibrary(db, userId) {
  ...
  return { currency: settings.results[0]?.currency || "€", initialized: Boolean(settings.results[0]?.initialized), items: items.results.map(x => ({ ...rowToItem(x), catalogReadDate: x.read_date || null, userStatus: map[x.id]?.status || null, readDate: map[x.id]?.readDate || null, ownRating: map[x.id]?.ownRating || null })) };
}
